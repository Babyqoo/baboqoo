...
import (
	...
	"github.com/baboqoo/go-baboqoo/accounts/usbwallet/internal/trezor"
...
// or whether the Baboqoo app was not started on it.
...
// Baboqoo address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.BaboqooAddress)
	if _, err := w.trezorExchange(&trezor.BaboqooGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.BaboqooTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.BaboqooTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
