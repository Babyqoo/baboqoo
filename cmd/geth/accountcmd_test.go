...
func TestAccountListEmpty(t *testing.T) {
	gbbq := runGeth(t, "account", "list")
	gbbq.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gbbq := runGeth(t, "account", "list", "--datadir", datadir)
	defer gbbq.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gbbq := runGeth(t, "account", "new", "--lightkdf")
	defer gbbq.ExpectExit()
	gbbq.Expect(`
...
`)
	gbbq.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	gbbq := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gbbq.ExpectExit()
	gbbq.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gbbq.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gbbq.StderrText(), m) {
...
// https://github.com/baboqoo/go-baboqoo/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gbbq.SetTemplateFunc("keypath", func(file string) string {
