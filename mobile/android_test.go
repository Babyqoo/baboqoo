...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gbbq.LightScryptN, Gbbq.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.baboqoo", new NodeConfig());
			node.start();
			...
			BaboqooClient ec = node.getBaboqooClient();
	...
	// signatures too. Regression test for go-baboqoo issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogbbqer
	...
	workspace, err := ioutil.TempDir("", "gbbq-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/baboqoo/gbbqtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gbbq and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.baboqoo", "github.com/baboqoo/go-baboqoo/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gbbq.aar"), "gbbq.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "baboqoo", "gbbqtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
