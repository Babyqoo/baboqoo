...
#
# The cluster contains a bootnode, a gbbq node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the gbbq and
# swarm nodes
start_bootnode() {
...
# start_gbbq_node starts a gbbq node with --datadir pointing at <base-dir>/gbbq
# and a single, unlocked account with password "gbbq"
start_gbbq_node() {
  local dir="${base_dir}/gbbq"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gbbq account address"
  fi
  ...
  start_node "gbbq" "${GETH_IP}" "$(which gbbq)" ${args[@]}
}
...
create_account() {
  ...
  gbbq --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
